...
class DLTKcat(nn.Module):
    ...
    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps, inv_Temp, Temp ):
        atoms_vector = self.comp_gat(atoms, adjacency) #(B,L, 40)
        amino_vector = self.prot_cnn( amino ) #(B,L, 40)

        super_feature = F.leaky_relu(torch.matmul(fps, self.fp0), 0.1)
        super_feature = F.leaky_relu(torch.matmul(super_feature, self.fp1), 0.1) #(B,40)
